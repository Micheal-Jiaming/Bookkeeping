...
| `.gitignore` / `.gitattributes` | 91 / 1 | `data/`, `dist/`, `build/`, `.venv/`, caches and **every plausible receipt format** ignored — not just `.jpg`/`.png` but `.heic`, `.heif`, `.jfif`, `.webp`, `.avif`, `.bmp`, `.tif`, `.tiff`, `.pdf` — with the four README screenshots re-included **by name, not by directory**; `* -text` |
