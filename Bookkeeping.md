...
| `.gitignore` / `.gitattributes` | 60 / 1 | `data/`, `dist/`, `build/`, `.venv/`, caches and **all receipt images** ignored, with the four README screenshots re-included **by name, not by directory**; `* -text` |
...
| 1.11.2 | 2026-09-03 | **The Payment field in `receipts.png` is blurred.** The user asked for it after the repository went public. The value was a card-shaped string masked to last-4 — not reproduced here, for the same reason it was blurred there — which I had checked the provenance of and deliberately left: it is invented fixture data from `tools/seed_demo.py` and `tools/make_sample_receipt.py`, the real receipt behind the project was paid in cash, and it is masked to last-4 anyway. That reasoning was beside the point. **A published image is judged on how it looks, not on where its data came from** -- a reader cannot audit `seed_demo.py` to reassure themselves, and a card-shaped string sitting beside real merchant names and the author's own name reads as real. Blurred with a radius large enough that no glyph survives magnification, with the input box's border left intact so it reads as deliberate rather than as a rendering fault. Recorded in `.gitignore` beside the capture instructions, because re-shooting the screenshot removes the blur and the next person to run `--tight` needs to know to redact again. Note that the same string is still visible in the five fixtures as source text; the user was told and scoped this to the image. |
